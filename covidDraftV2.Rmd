...
deltaPositive <- subset(deltaSummary, delta == TRUE)

# Plotting the data per region 
ggplot(deltaPositive, aes(x = collection_date, y = lineage_frequency)) +
  geom_line() +
  facet_wrap(~region) +
  labs(title = "Delta Frequencies by Region", 
       x = "Date", 
       y = "Frequency") +
  theme_minimal()
```
