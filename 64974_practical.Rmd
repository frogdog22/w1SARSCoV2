...
##so this new data frame lets you calculate the amount of infections per date for every lineage
...
## So this code is subsetting the lineage summary dataframe based on the major lineages - only looking at delta and alpha, plot based on data and number of infections of each lineage
## if we have to do this for the final assessment maybe get the colour scheme set up beforehand so changing the lineage also changes the colour
## stochasticity in the graph could be due to variations in testing - ONS samples a subpopulation of the UK pop - inherant stochastisity when looking at daily cases
## if you looked at weekly intervals it might be less noisey - doing this later on in the practical 
```
...
##e.g. 2021 would expect almost all or all cases sampled to be delta. Public health implications - effect on R0, designing vaccines
##Predominantly tracking variants as we care about their properties - if the virus have mutations that make it better at transmissing or escaping immunity - want to understand every change in the virus that might have an implication for public health
##was it luck or did this variant have a selective advantage over other variants
##striking pattern of very rapid lineage replacements 
##consecutive waves which all reach fixation and became the most dominant 
```
...
##this code is binning the data into 10 day intervals - by doing this there is a smoother frequency plot - this matters for calculating the growth advantage of a lineage over another - woud like the data to be least affected by noise, makes you less susceptible to to make biased judments of growth advantage due to sampling 
```
...
##increasing pop size makes it take longer. here we are tracking variant frequencies - so any paricipants that test negative are not included in the value of N. whilst the population of participants remains the same here population size is the number of cases with a given varient - this doesn't really remain constant - so take the equation with a pinch of salt
s <- 0.1          # Selection coefficient
##increasing s makes fixation be reached faster
f0 <- 1 / N       # Initial frequency of the variant
##changing this doesn't seem to change anything?
tmax <- 200       # Maximum number of generations
##increasing this makes fixation reached faster
...
##looking at trajectory of delta, putting it in the right time interval to measure the growth advantage of delta - this equation only really works in the growth phase
...
##the right time interval to use as an imput for the growth calc to get growth advantage - from frequency of delta being 0 to the point of fixation - frequency of 100
##if you gave the whole trajectory it would be meaningless as it would be including delta going down
##point of this session we are trying to understand how models work with a real data set 
```
...
##this splits out delta trajectory from the point where it is 0 to where it is 100
##the first non 0 value was the input
##could try putting different values in and seeing if the output changes
...
## does it look like logistic growth - what would make it not look like logisitc growth? what does the initial non 0 value of delta mean - did it start in the UK or was it imported into the UK? can you tell the difference?
## if there was location data about where these sequences were collected across the UK you could maybe tell if it was imported in or originated in UK
## can you tell uk vs imported from this data alone? if imported there could be multiple different locations depending on where it was prevalent outside of the UK - but it might look like it just emerged from a location in the UK if people came from abroad just to one place
## looking at aggregate frequencies here - don't have postcodes - but would be able to see that different geographic areas might have different frequencies of this
## ONS not the quickest indicator of cases of delta - ONS is for people with fixed home addresses - different surveillance systems would give faster indication of if a variant is emerging in the UK or not
## if to repeat exercise but not use smooth 10 day frequency intervals, if used daily jittering frequencies would not have made the same inference about the growth rate - may have underestimated growth advantage due to lots of 0/close to 0 frequencies early on due to stochasticity - averaging across longer windows prevents results being driven by the artefacts of sample
## simple equation based on exponential growth assumption knowing the selective advantagae of delta e.g. saying cases are likely to double every 5 days - use logisitc growth model as early stages of this look like exponential growth 
## could a new variant growing in a pop be driven entirely by luck, have no advantage but still growing - e.g. if it got to a subset of the pop that was completley naive and hadn't been exposed to alpha - would give the impression of delta having a large selective advantage, but in reality it's just a founder effect - stochastic affect tied to geographical location - this was debated for the first few months of alpha(?) spreading in the UK
## this is simple maths with huge implications 
## (if cases has dropped during lockdown which they didn't) lockdown may give impression of no growth advantage of alpha when actually cases are just going down to lockdown - lots of other factors affecting growth advantage calculations - lockdown, masks, social distancing 
## As we moved on through pandemic - e.g. omicron emergence a good chuck of population already infected, lots had vaccination, lots had both vaccination and infected - lots of immunity, confering a growth advantage is dependent on this and immune evasion too
## different dynamics in different countries/different strains - what we infer may not be directly transferrable 
## delta and omicron (not alpha) were dominant globally (alpha dominant uk europe, africa had beta, south america gamma))
## late 22/early 23 multiple cocirculating lineages - none of them reached fixation - by definition violates the assumption of the simple logistic growth model used here - not something covered in this practical but would have to use a generalised linear regression model which allows you to include multiple lineages, or compartmentalized SIR models - however these demant a lot of parameters to estimate them - logistic growth model requires the least amount of prior knowledge. Advantages and limitations of different models should stay aware of.
## reinfection with a different variant is a function of how much immunity already established - heterogeneity in the immunity of a population - some, like elderly may have more frail immunity and be more susceptible to reinfection - sometimes simple logistic model is the best avalialble 
```
